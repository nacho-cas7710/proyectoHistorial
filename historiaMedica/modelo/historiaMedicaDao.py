...
def listarHistoria(idPersona):
    ...
    try:
        ...
        mensaje ='Error el listar historia medica'
...
def eliminarHistoria (idHitoriaMedica):
    conexiom = ConexionDB()
    sql = f'DELETE FROM historiaMedica WHERE idHistoriaMedica = {idHitoriaMedica}'

    try:
        conexiom.cursor.execute(sql)
        conexiom.cerrarConexion()
